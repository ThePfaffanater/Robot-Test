...
public class DrivePlan extends ArrayList<Waypoint>{
    ...
    public long getPointThroughPointTime(int index1, int index2) {
        ...
        for(int i = (index1+1); i < index2; i++){ //Gets the distance and divides by velocity to get time.
            ...
            double dist= Math.sqrt(Math.pow(point2.getX().doubleValue()-point1.getX().doubleValue(), 2) + Math.pow(point2.getZ().doubleValue()-point1.getZ().doubleValue(), 2) + Math.pow(point2.getY().doubleValue()-point1.getY().doubleValue(), 2));
    ...
    public double getPointThroughPointDist(int index1, int index2){
        ...
        for(int i = (index1+1); i < index2; i++){
            Waypoint point1 = this.get(i-1);
            Waypoint point2 = this.get(i);
            fullDist+= Math.sqrt(Math.pow(point2.getX().doubleValue()-point1.getX().doubleValue(), 2) + Math.pow(point2.getZ().doubleValue()-point1.getZ().doubleValue(), 2) + Math.pow(point2.getY().doubleValue()-point1.getY().doubleValue(), 2));
            //^^ |P1P2|=√(x2−x1)2+(y2−y1)2+(z2−z1) the 3D dist formula.
        }
